...
public:
    ...
    {
        return ((levelValue & 0xFFFF) << 16) + ((levelType & 0xFF) << 8)
            + dataType;
    }
...
public:
    ...
    static GribRecord* InterpolatedRecord(const GribRecord& rec1,
        const GribRecord& rec2, double d, bool dir = false);
    ...
    static GribRecord* MagnitudeRecord(
    ...
    double getInterpolatedValue(double px, double py,
        bool numericalInterpolation = true, bool dir = false) const;
...
protected:
    ...
    static bool GetInterpolatedParameters(const GribRecord& rec1,
        const GribRecord& rec2, double& La1, double& Lo1, double& La2,
        double& Lo2, double& Di, double& Dj, int& im1, int& jm1, int& im2,
        int& jm2, int& Ni, int& Nj, int& rec1offi, int& rec1offj, int& rec2offi,
        int& rec2offj);
