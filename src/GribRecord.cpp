...
{
    printf("%d: idCenter=%d idModel=%d idGrid=%d dataType=%d levelType=%d "
           "levelValue=%d hr=%f\n",
...
bool GribRecord::GetInterpolatedParameters(const GribRecord& rec1,
    const GribRecord& rec2, double& La1, double& Lo1, double& La2, double& Lo2,
    double& Di, double& Dj, int& im1, int& jm1, int& im2, int& jm2, int& Ni,
    int& Nj, int& rec1offi, int& rec1offj, int& rec2offi, int& rec2offj)
{
    ...
    if (i
        == iiters) // failed to align, would need spacial interpolation to work
        return false;
...
//-------------------------------------------------------------------------------
GribRecord* GribRecord::InterpolatedRecord(
    const GribRecord& rec1, const GribRecord& rec2, double d, bool dir)
{
    ...
    if (!GetInterpolatedParameters(rec1, rec2, La1, Lo1, La2, Lo2, Di, Dj, im1,
            jm1, im2, jm2, Ni, Nj, rec1offi, rec1offj, rec2offi, rec2offj))
        return NULL;
...
GribRecord* GribRecord::Interpolated2DRecord(GribRecord*& rety,
    const GribRecord& rec1x, const GribRecord& rec1y, const GribRecord& rec2x,
    const GribRecord& rec2y, double d)
{
    ...
    if (!GetInterpolatedParameters(rec1x, rec2x, La1, Lo1, La2, Lo2, Di, Dj,
            im1, jm1, im2, jm2, Ni, Nj, rec1offi, rec1offj, rec2offi, rec2offj))
        return NULL;
...
GribRecord* GribRecord::MagnitudeRecord(
    ...
    if (rec1.data && rec2.data && rec1.Ni == rec2.Ni && rec1.Nj == rec2.Nj) {
        ...
        for (int i = 0; i < size; i++)
            if (rec1.data[i] == GRIB_NOTDEF || rec2.data[i] == GRIB_NOTDEF)
                ...
                rec->data[i]
                    = sqrt(pow(rec1.data[i], 2) + pow(rec2.data[i], 2));
    } else
        rec->ok = false;
...
void GribRecord::setRecordCurrentDate(time_t t)
{
    ...
    sprintf(
        strCurDate, "%04d-%02d-%02d %02d:%02d", year, month, day, hour, minute);
}
    ...
    zuint year, zuint month, zuint day, zuint hour, zuint min, zuint sec)
{
    ...
    wxDateTime dt(date);
    temps = dt.GetTicks();
    temps += ((hour * 3600) + (min * 60)
        + sec); // find datetime exactly as in the file
    wxDateTime dtt(temps);
    wxTimeSpan of = wxDateTime::Now()
        - (wxDateTime::Now().ToGMT()); // transform to local time
...
double GribRecord::getInterpolatedValue(
    double px, double py, bool numericalInterpolation, bool dir) const
{
    if (!ok || Di == 0 || Dj == 0)
        return GRIB_NOTDEF;
...
bool GribRecord::getInterpolatedValues(double& M, double& A,
    const GribRecord* GRX, const GribRecord* GRY, double px, double py,
