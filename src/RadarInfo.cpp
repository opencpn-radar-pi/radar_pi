...
void RadarInfo::ResetSpokes() {
  ...
  if (m_draw_panel.draw) {
    for (size_t r = 0; r < m_spokes; r++) {
      m_draw_panel.draw->ProcessRadarSpoke(0, r, zap, m_spoke_len_max, pos, false);
    }
  }
  if (m_draw_overlay.draw) {
    for (size_t r = 0; r < m_spokes; r++) {
      m_draw_overlay.draw->ProcessRadarSpoke(0, r, zap, m_spoke_len_max, pos, true);
    }
  }
...
void RadarInfo::ProcessRadarSpoke(SpokeBearing angle, SpokeBearing bearing, uint8_t *data, size_t len, int range_meters,
  ...
  wxCriticalSectionLocker lock(m_lock_last_spoke);
  ...
  if (m_draw_overlay.draw && !draw_trails_on_overlay) {
    m_draw_overlay.draw->ProcessRadarSpoke(M_SETTINGS.overlay_transparency.GetValue(), bearing, data, len, m_history[bearing].pos, true);
  }
  ...
  if (m_draw_panel.draw) {
    m_draw_panel.draw->ProcessRadarSpoke(4, stabilized_mode ? bearing : angle, data, len, m_history[bearing].pos,false);
  }
}
...
void RadarInfo::SetAutoRangeMeters(int autorange_to_set) {
  ...
  LOG_VERBOSE(wxT(" Automatic range changed 1 from %i to %i meters, m_range.GetValue()=%i"), m_previous_auto_range_meters, meters, m_range.GetValue());
  if (m_range.GetValue() != m_previous_auto_range_meters) {
    LOG_VERBOSE(wxT(" m_previous_auto_range_meters=%i, m_range.GetValue()=%i"), m_previous_auto_range_meters, m_range.GetValue());
  }
  if (m_state.GetValue() == RADAR_TRANSMIT && m_range.GetState() == RCS_AUTO_1 && m_control) {
    ...
    if (test < 90 || test > 110) {  //   range change required
      if (meters != m_range.GetValue()) {
        LOG_VERBOSE(wxT("%s, Automatic range changed from %d to %d meters"), m_name, m_previous_auto_range_meters, meters);
