...
int br24radar_pi::Init(void)
{
    ...
    m_tool_id = InsertPlugInTool(
        wxT(""), _img_radar_red, _img_radar_red, wxITEM_NORMAL, wxT("BR24Radar"), wxT(""), NULL, BR24RADAR_TOOL_POSITION, 0, this);
...
void br24radar_pi::DoTick(void)
{
    ...
    for (size_t r = 0; r < RADARS; r++) {
        if (m_radar[r]->radar_seen && TIMER_ELAPSED(now, m_radar[r]->radar_watchdog)) {
    ...
    for (size_t r = 0; r < RADARS; r++) {
        if (m_radar[r]->data_seen && TIMER_ELAPSED(now, m_radar[r]->data_watchdog)) {
            ...
            if (TimedTransmit_IdleBoxMode == 1) { // Idle
                ...
                if (m_idle_dialog_time_left != time_left) {
                    br24radar_pi::m_pIdleDialog->SetIdleTimes(
                        TimedTransmit_IdleBoxMode, m_settings.timed_idle * factor / 60, time_left);
                    m_pIdleDialog->Show();
...
bool br24radar_pi::RenderGLOverlay(wxGLContext *pcontext, PlugIn_ViewPort *vp)
{
    ...
    if (!m_opencpn_gl_context && !m_opencpn_gl_context_broken) {
        wxLogMessage(
            wxT("BR24radar_pi: OpenCPN does not pass OpenGL context. "
    ...
    if (m_radar[m_settings.chart_overlay]->auto_range_mode) {
        ...
        if (test < 95 || test > 105) { //   range change required
            ...
            size_t idx = convertMetersToRadarAllowedValue(
                &displayedRange, m_settings.range_units, m_radar[m_settings.chart_overlay]->radar_type);
...
void br24radar_pi::HandleBogeyCount(int *bogey_count)
{
    ...
    if (bogeysFound) {
        ...
        if (m_settings.timed_idle != 0)
            m_radar[radar]->control_dialog->SetTimedIdleIndex(0); // Disable Timed Idle if set
    ...
    if (pConf) {
        ...
        for (int r = 0; r < RADARS; r++) {
            ...
            for (int i = 0; i < GUARD_ZONES; i++) {
                int v;
                pConf->Read(
                    wxString::Format(wxT("Radar%dZone%dStartBearing"), r, i), &m_radar[r]->guard_zone[i]->start_bearing, 0.0);
        ...
        default: {
            if (m_radar[radar]->radar_seen) {
                if (!m_radar[radar]->SetControlValue(controlType, value)) {
                    wxLogMessage(
                        wxT("BR24radar_pi: %s unhandled control setting for control %d"), m_radar[radar]->name, controlType);
                }
            }
        }
    }
