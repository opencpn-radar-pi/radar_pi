...
static double local_distance(double lat1, double lon1, double lat2,
                             double lon2) {
...
static double radar_distance(double lat1, double lon1, double lat2, double lon2,
                             char unit) {
...
int br24radar_pi::Init(void) {
  ...
  if (r != 0) {
    wxLogError(
        wxT("BR24radar_pi: Unable to initialise Windows Sockets, error %d"), r);
  ...
  if (LoadConfig()) {
    ...
    wxLogMessage(wxT("BR24radar_pi: Log verbosity = %d (to modify, set "
                     "VerboseLog to 0..4)"),
                 m_settings.verbose);
  } else {
    wxLogMessage(
        wxT("BR24radar_pi: configuration file values initialisation failed"));
    return 0; // give up
  }
  ComputeColorMap();
  ...
  m_tool_id = InsertPlugInTool(wxT(""), _img_radar_red, _img_radar_red,
                               wxITEM_NORMAL, wxT("BR24Radar"), wxT(""), NULL,
  ...
  if (!m_radar[1]->receive && m_settings.enable_dual_radar &&
      m_radar[0]->radar_type == RT_4G) {
    m_radar[1]->StartReceive();
    m_radar[0]->SetName(_("Radar A"));
  }

  return (WANTS_DYNAMIC_OPENGL_OVERLAY_CALLBACK |
          WANTS_OPENGL_OVERLAY_CALLBACK | WANTS_OVERLAY_CALLBACK |
          WANTS_TOOLBAR_CALLBACK | INSTALLS_TOOLBAR_TOOL |
          INSTALLS_CONTEXTMENU_ITEMS | USES_AUI_MANAGER | WANTS_CONFIG |
...
void br24radar_pi::ShowRadarControl(int radar, bool show) {
  if (m_settings.verbose >= 2) {
    wxLogMessage(wxT("BR24radar_pi: ShowRadarControl(%d, %d)"), radar,
                 (int)show);
  }
  ...
  if (show) {
    ...
    if (!m_radar[radar]->control_dialog) {
      ...
      m_radar[radar]->control_dialog->Create(m_parent_window, this,
                                             m_radar[radar]);
      m_radar[radar]->control_dialog->SetPosition(
          m_dialogLocation[DL_CONTROL + radar].pos);
      m_radar[radar]->control_dialog->Fit();
      m_radar[radar]->control_dialog->Hide();
      int range = m_radar[radar]->range_meters;
      int idx = convertMetersToRadarAllowedValue(&range, m_settings.range_units,
                                                 m_radar[radar]->radar_type);
  ...
  m_pMessageBox->UpdateMessage(
      m_opengl_mode, m_bpos_set, m_heading_source != HEADING_NONE,
      m_var_source != VARIATION_SOURCE_NONE, m_radar[radar]->radar_seen,
...
void br24radar_pi::OnControlDialogClose(RadarInfo *ri) {
  if (ri->control_dialog) {
    m_dialogLocation[DL_CONTROL + ri->radar].pos =
        ri->control_dialog->GetPosition();
...
void br24radar_pi::ShowGuardZoneDialog(int radar, int zone) {
  if (!m_pGuardZoneDialog) {
    ...
    m_pGuardZoneDialog->Create(m_parent_window, this, wxID_ANY,
                               _(" Guard Zone Control"),
                               m_dialogLocation[DL_CONTROL].pos);
  }
  if (zone >= 0) {
    m_dialogLocation[DL_CONTROL + radar].pos =
        m_radar[radar]->control_dialog->GetPosition();
...
void br24radar_pi::ComputeColorMap() {
  ...
  case 1:
    for (int i = 0; i <= UINT8_MAX; i++) {
      m_color_map[i] =
          (i >= m_settings.threshold_red)
              ? BLOB_RED
              : (i >= m_settings.threshold_green)
                    ? BLOB_GREEN
...
void br24radar_pi::DoTick(void) {
  ...
  if (m_settings.verbose >= 2) {
    ...
    if (now - refresh_indicator >= 1) {
      ...
      wxLogMessage(wxT("BR24radar_pi: number of refreshes last second = %d"),
                   performance_counter);
  ...
  m_refresh_rate =
      REFRESHMAPPING[m_settings.refreshrate - 1]; // set actual refresh rate
  ...
  if (m_var_source != VARIATION_SOURCE_NONE &&
      TIMER_ELAPSED(now, m_var_watchdog)) {
    m_var_source = VARIATION_SOURCE_NONE;
  ...
  for (size_t r = 0; r < RADARS; r++) {
    if (m_radar[r]->radar_seen &&
        TIMER_ELAPSED(now, m_radar[r]->radar_watchdog)) {
  ...
  for (size_t r = 0; r < RADARS; r++) {
    if (m_radar[r]->data_seen &&
        TIMER_ELAPSED(now, m_radar[r]->data_watchdog)) {
  ...
  if (any_data_seen) { // Something coming from radar unit?
    ...
    if (m_settings.show_radar ==
        RADAR_ON) { // if not, radar will time out and go standby
  ...
  if (m_settings.timed_idle != 0 &&
      m_toolbar_button) { // Reset function if radar connection is lost RED==0
    ...
    if (m_init_timed_transmit) { // Await user finalizing idle time option set.
      if (m_toolbar_button == TB_GREEN) {
        ...
        if (TT_now > (m_idle_watchdog + (m_settings.idle_run_time * 60)) ||
        ...
        if (TT_now > (m_idle_watchdog + (m_settings.timed_idle * factor)) ||
            m_idle_dialog_time_left == 999) {
      ...
      if (TimedTransmit_IdleBoxMode == 1) { // Idle
        time_left =
            ((m_idle_watchdog + (m_settings.timed_idle * factor)) - TT_now) /
            60;
        if (m_idle_dialog_time_left != time_left) {
          br24radar_pi::m_pIdleDialog->SetIdleTimes(
              TimedTransmit_IdleBoxMode, m_settings.timed_idle * factor / 60,
              time_left);
          m_pIdleDialog->Show();
      ...
      if (TimedTransmit_IdleBoxMode == 2) { // Transmit
        time_left =
            ((m_idle_watchdog + (m_settings.idle_run_time * 60)) - TT_now) / 60;
        if (m_idle_dialog_time_left != time_left) {
          br24radar_pi::m_pIdleDialog->SetIdleTimes(
              TimedTransmit_IdleBoxMode, m_settings.idle_run_time, time_left);
          m_pIdleDialog->Show();
          m_idle_dialog_time_left = time_left;
        }
      }
      ...
      if (m_radar[radar]->control_dialog) {
        if (m_radar[radar]->control_dialog->topSizer->IsShown(
                m_radar[radar]->control_dialog->controlBox)) {
          m_init_timed_transmit = true; // First time init: Await user to leave
                                        // Timed transmit setting menu.
          m_idle_watchdog = TT_now;
        }
      }
    }
...
bool br24radar_pi::RenderGLOverlay(wxGLContext *pcontext, PlugIn_ViewPort *vp) {
  ...
  if (!m_opencpn_gl_context && !m_opencpn_gl_context_broken) {
    wxLogMessage(wxT("BR24radar_pi: OpenCPN does not pass OpenGL context. "
                     "Resize of OpenCPN window may be broken!"));
  }
  ...
  // from DoTick().
  double max_distance =
      radar_distance(vp->lat_min, vp->lon_min, vp->lat_max, vp->lon_max, 'm');
  ...
  if (m_settings.chart_overlay < 0 ||
      m_settings.show_radar == RADAR_OFF) { // No overlay desired
    return true;
  }
  ...
  if (m_radar[m_settings.chart_overlay]->auto_range_mode) {
    ...
    if (test < 95 || test > 105) { //   range change required
      if (m_settings.verbose) {
        wxLogMessage(
            wxT("BR24radar_pi: Automatic range changed from %d to %d meters"),
      ...
      size_t idx = convertMetersToRadarAllowedValue(
          &displayedRange, m_settings.range_units,
          m_radar[m_settings.chart_overlay]->radar_type);
      if (displayedRange !=
          m_radar[m_settings.chart_overlay]->commanded_range_meters) {
  ...
  GetCanvasLLPix(vp, wxPoint(0, vp->pix_height - 1), &ulat,
                 &ulon); // is pix_height a mapable coordinate?
  GetCanvasLLPix(vp, wxPoint(0, 0), &llat, &llon);
  dist_y = radar_distance(llat, llon, ulat, ulon,
                          'm'); // Distance of height of display - meters
  pix_y = vp->pix_height;
  v_scale_ppm = 1.0;
  if (dist_y > 0.) {
    // v_scale_ppm = vertical pixels per meter
    v_scale_ppm = vp->pix_height /
                  dist_y; // pixel height of screen div by equivalent meters
  }

  if (m_settings.verbose >= 4) {
    wxLogMessage(wxT("BR24radar_pi: RenderRadarOverlay lat=%g lon=%g "
                     "v_scale_ppm=%g rotation=%g skew=%g scale=%f"),
                 vp->clat, vp->clon, vp->view_scale_ppm, vp->rotation, vp->skew,
                 vp->chart_scale);
  }
  if (m_radar[m_settings.chart_overlay]->data_seen) {
    RenderRadarOverlay(
        boat_center, v_scale_ppm,
        rad2deg(vp->rotation + vp->skew * m_settings.skew_factor));
  }
  m_refresh_busy_or_queued = false;
  return true;
}

void br24radar_pi::RenderRadarOverlay(wxPoint radar_center, double v_scale_ppm,
                                      double rotation) {
  RadarInfo *ri = m_radar[m_settings.chart_overlay];

  rotation = MOD_DEGREES(
      rotation // viewport rotation + skew (in degrees now)
  ...
  if (meters) {
    ...
    double scale_factor =
        v_scale_ppm / radar_pixels_per_meter; // screen pix/radar pix
...
void br24radar_pi::ScanGuardZones(int max_range, int AB) {
  ...
  for (size_t z = 0; z < GUARD_ZONES; z++) {
    ...
    case GZ_ARC:
      ...
      begin_arc = SCALE_DEGREES_TO_RAW2048(
          begin_arc); // m_hdt added to provide guard zone relative to heading
    ...
    for (int angle = begin_arc; angle < end_arc; angle++) {
      ...
      for (int radius = 0; radius <= RETURNS_PER_LINE - 2; ++radius) {
        ...
        int inner_range =
            m_radar[AB]->guard_zone[z]->inner_range; // now in meters
        int outer_range =
            m_radar[AB]->guard_zone[z]->outer_range; // now in meters
        int bogey_range = radius * max_range / RETURNS_PER_LINE;
        if (bogey_range > inner_range &&
            bogey_range <
                outer_range) { // within range, now check requirement for alarm
          if ((m_settings.multi_sweep_filter[AB][z]) !=
              0) { // multi sweep filter on for this z
            GLubyte hist = scan->history[radius] & 7; // check only last 3 bits
            if (!(hist == 3 ||
                  hist >=
                      5)) { // corresponds to the patterns 011, 101, 110, 111
            ...
            if (strength <= displaysetting_threshold[m_settings.display_option])
              continue;
          }
          int index = z + 2 * AB;
          m_bogey_count[index]++;
...
void br24radar_pi::HandleBogeyCount(int *bogey_count) {
  ...
  if (m_settings.verbose >= 2) {
    wxLogMessage(wxT("BR24radar_pi: handle bogeycount bogeysFound %d"),
                 bogeysFound);
  }
  ...
  if (bogeysFound) {
    ...
    if (m_settings.timed_idle != 0)
      m_radar[radar]->control_dialog->SetTimedIdleIndex(
          0); // Disable Timed Idle if set
    ...
    if (m_pGuardZoneBogey) {
      m_pGuardZoneBogey->SetBogeyCount(
          bogey_count, m_guard_bogey_confirmed ? -1 : ALARM_TIMEOUT - delta_t);
    }
  }

  if (!bogeysFound && m_pGuardZoneBogey) {
    m_pGuardZoneBogey->SetBogeyCount(
        bogey_count, -1); // with -1 "next alarm in... "will not be displayed
...
bool br24radar_pi::LoadConfig(void) {
  ...
  if (pConf) {
    ...
    pConf->Read(wxT("RangeUnits"), &m_settings.range_units,
                0); // 0 = "Nautical miles"), 1 = "Kilometers"
    ...
    pConf->Read(wxT("Transparency"), &m_settings.overlay_transparency,
    ...
    pConf->Read(wxT("GuardZonesThreshold"), &m_settings.guard_zone_threshold,
                5L);
    pConf->Read(wxT("GuardZonesRenderStyle"),
                &m_settings.guard_zone_render_style, 0);
    ...
    for (int r = 0; r < RADARS; r++) {
      pConf->Read(wxString::Format(wxT("Radar%dRotation"), r),
                  &m_radar[r]->rotation.value, 0);
      for (int i = 0; i < GUARD_ZONES; i++) {
        int v;
        pConf->Read(wxString::Format(wxT("Radar%dZone%dStartBearing"), r, i),
                    &m_radar[r]->guard_zone[i]->start_bearing, 0.0);
        pConf->Read(wxString::Format(wxT("Radar%dZone%dEndBearing"), r, i),
                    &m_radar[r]->guard_zone[i]->end_bearing, 0.0);
        pConf->Read(wxString::Format(wxT("Radar%dZone%dOuterRange"), r, i),
                    &m_radar[r]->guard_zone[i]->outer_range, 0);
        pConf->Read(wxString::Format(wxT("Radar%dZone%dInnerRange"), r, i),
                    &m_radar[r]->guard_zone[i]->inner_range, 0);
        pConf->Read(wxString::Format(wxT("Radar%dZone%dType"), r, i), &v, 0);
        m_radar[r]->guard_zone[i]->type = (GuardZoneType)v;
        pConf->Read(wxString::Format(wxT("Radar%dZone%dFilter"), r, i),
                    &m_radar[r]->guard_zone[i]->multi_sweep_filter, 0);
      }
    }
    ...
    pConf->Read(wxT("ThresholdMultiSweep"), &m_settings.threshold_multi_sweep,
                20);
...
bool br24radar_pi::SaveConfig(void) {
  ...
  if (pConf) {
    ...
    pConf->Write(wxT("GuardZonesRenderStyle"),
                 m_settings.guard_zone_render_style);
    ...
    for (size_t i = 0; i < ARRAY_SIZE(m_dialogLocation); i++) {
      if (m_dialogLocation[i].pos.x) {
        pConf->Write(wxString::Format(wxT("Control%dPosX"), i),
                     m_dialogLocation[i].pos.x);
      }
      if (m_dialogLocation[i].pos.y) {
        pConf->Write(wxString::Format(wxT("Control%dPosY"), i),
                     m_dialogLocation[i].pos.y);
      }
      if (m_dialogLocation[i].size.x) {
        pConf->Write(wxString::Format(wxT("Control%dSizeX"), i),
                     m_dialogLocation[i].size.x);
      }
      if (m_dialogLocation[i].size.y) {
        pConf->Write(wxString::Format(wxT("Control%dSizeY"), i),
                     m_dialogLocation[i].size.y);
      }
    }

    for (size_t r = 0; r < RADARS; r++) {
      pConf->Write(wxString::Format(wxT("Radar%dRotation"), r),
                   m_radar[r]->rotation.value);
      for (size_t i = 0; i < GUARD_ZONES; i++) {
        pConf->Write(wxString::Format(wxT("Radar%dZone%dStartBearing"), r, i),
                     m_radar[r]->guard_zone[i]->start_bearing);
        pConf->Write(wxString::Format(wxT("Radar%dZone%dEndBearing"), r, i),
                     m_radar[r]->guard_zone[i]->end_bearing);
        pConf->Write(wxString::Format(wxT("Radar%dZone%dOuterRange"), r, i),
                     m_radar[r]->guard_zone[i]->outer_range);
        pConf->Write(wxString::Format(wxT("Radar%dZone%dInnerRange"), r, i),
                     m_radar[r]->guard_zone[i]->inner_range);
        pConf->Write(wxString::Format(wxT("Radar%dZone%dType"), r, i),
                     (int)m_radar[r]->guard_zone[i]->type);
        pConf->Write(wxString::Format(wxT("Radar%dZone%dFilter"), r, i),
                     m_radar[r]->guard_zone[i]->multi_sweep_filter);
      }
    }
...
void br24radar_pi::SetPositionFixEx(PlugIn_Position_Fix_Ex &pfix) {
  ...
  if (m_var_source <= VARIATION_SOURCE_FIX && !wxIsNaN(pfix.Var) &&
      (fabs(pfix.Var) > 0.0 || m_var == 0.0)) {
    if (m_var_source < VARIATION_SOURCE_FIX || fabs(pfix.Var - m_var) > 0.05) {
      wxLogMessage(
          wxT("BR24radar_pi: Position fix provides new magnetic variation %f"),
          pfix.Var);
      if (m_pMessageBox) {
        if (m_pMessageBox->IsShown()) {
          info = _("GPS");
  ...
  if (m_settings.verbose >= 2) {
    wxLogMessage(wxT("BR24radar_pi: SetPositionFixEx var=%f "
                     "heading_on_radar=%d var_wd=%d show_radar=%d"),
                 pfix.Var, m_heading_on_radar,
                 TIMER_NOT_ELAPSED(now, m_var_watchdog), m_settings.show_radar);
  }
...
void br24radar_pi::SetPluginMessage(wxString &message_id,
    ...
    if (!reader.Parse(message_body, &message)) {
      ...
      if (variation != 360.0) {
        if (m_var_source != VARIATION_SOURCE_WMM) {
          wxLogMessage(wxT("BR24radar_pi: WMM plugin provides new magnetic "
                           "variation %f"),
                       variation);
        }
        m_var = variation;
...
void br24radar_pi::SetControlValue(
    int radar, ControlType controlType,
    int value) { // sends the command to the radar
  switch (controlType) {
  ...
  default: {
    if (m_radar[radar]->radar_seen) {
      if (!m_radar[radar]->SetControlValue(controlType, value)) {
        wxLogMessage(
            wxT("BR24radar_pi: %s unhandled control setting for control %d"),
            m_radar[radar]->name, controlType);
      }
    }
  }
  }
...
//*****************************************************************************************************
void br24radar_pi::CacheSetToolbarToolBitmaps(int bm_id_normal,
                                              int bm_id_rollover) {
  if ((bm_id_normal == m_sent_bm_id_normal) &&
      (bm_id_rollover == m_sent_bm_id_rollover)) {
    return; // no change needed
  }

  if ((bm_id_normal == -1) ||
      (bm_id_rollover == -1)) { // don't do anything, caller's responsibility
...
void br24radar_pi::SetNMEASentence(wxString &sentence) {
  ...
  if (m_NMEA0183.PreParse()) {
    if (m_NMEA0183.LastSentenceIDReceived == _T("HDG") && m_NMEA0183.Parse()) {
      if (m_settings.verbose >= 2) {
        wxLogMessage(wxT("BR24radar_pi: received HDG variation=%f "
                         "var_source=%d m_var=%f"),
                     m_NMEA0183.Hdg.MagneticVariationDegrees, m_var_source,
                     m_var);
      }
      if (!wxIsNaN(m_NMEA0183.Hdg.MagneticVariationDegrees) &&
          (m_var_source <= VARIATION_SOURCE_NMEA ||
           (m_var == 0.0 && m_NMEA0183.Hdg.MagneticVariationDegrees > 0.0))) {
        double newVar;
        ...
        if (m_settings.verbose && fabs(newVar - m_var) >= 0.1) {
          wxLogMessage(
              wxT("BR24radar_pi: NMEA provides new magnetic variation %f"),
              newVar);
        }
        m_var = newVar;
        m_var_source = VARIATION_SOURCE_NMEA;
      ...
      if (m_heading_source == HEADING_HDM &&
          !wxIsNaN(m_NMEA0183.Hdg.MagneticSensorHeadingDegrees)) {
...
if ((m_settings.show_radar == RADAR_ON && m_bpos_set &&
     m_heading_source != HEADING_NONE && m_data_seen) ||
    (m_settings.emulator_on && m_settings.show_radar == RADAR_ON)) {
  if (m_range_meters[m_settings.selectRadarB] > 0 &&
      m_scanner_state == RADAR_ON) {
    // Guard Section
    for (int i = 0; i < 4; i++) {
      m_bogey_count[i] = 0;
    ...
    if (m_settings.selectRadarB == 0)
      metersA = meters;
    if (m_settings.selectRadarB == 1)
      metersB = meters;
