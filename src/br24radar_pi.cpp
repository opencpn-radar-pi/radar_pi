 ...
 *
 * Project:  OpenCPN
 * Purpose:  SIMRAD BR24 Radar Plugin
 * Author:   David Register
 * Pagiarized by:	Dave Cowell
 *
 ***************************************************************************
...
// Range Calibration Factor
	...
	wxString m_temp;
	m_temp.Printf(_T("%2.5f°"), br_range_calibration);
...
{
      br_auto = (mode == 1);
	  if (!br_auto) m_pManualDialog->Show();
}
...
void br24radar_pi::draw_blob_gl(double angle, double radius, double blob_r, double arc_length)
{
	  ...
	  if (blob_width < blob_r) blob_width = blob_r;
...
void br24radar_pi::TransmitCmd(char* msg, int size)
{
	  ...
	  {
			  wxSocketError err = m_out_sock101->LastError();
			  wxLogMessage(_T("BR24 m_out_sock101 error \n"));
	  };
};
...
//      m_pTarget = MessageTarget;
      ...
      m_sock = 0;
		  ...
		  wxString msg = (_T("RXThread: Creating thread m_IP addr: %d m_Port: %d"), IP_addr, service_port);
		  wxLogMessage(msg);
...
//*
	  if(m_sock) {
		  msg = (_T("RXThreadEntry: Datagram Socket opened: Receiver %d"), m_service_port);
	  }
	  else {
		  msg = (_T("RXThreadEntry: Datagram Socket failed: Receiver %d"), m_service_port);
	  }
	  wxLogMessage(msg);
//*/
...
/*
      ...
      if(bam) {
            ...
            msg = _T("RXThreadEntry: Successfully added to multicast group. ");
      }
      else {
            printf("   Failed to add to multicast group \n");
            msg = _T("RXThreadEntry: Failed to add to multicast group.");
      }
      wxLogMessage(msg);
*/
...
//		memcpy(dest_data2, br_line.data, 512);				// both lines get same scanner data
		...
		br_scan_buf[angle][512] = (byte)0xff;
...
//				outfile.write((const char*)dest_data1, 512); // data line copied to scan buffer.
			...
			for(int angle=0 ; angle < 358 ; angle += 2)
			{
					...
					{
						alpha = br_scan_buf[angle][radius];
						alpha2 = br_scan_buf[angle+2][radius];
