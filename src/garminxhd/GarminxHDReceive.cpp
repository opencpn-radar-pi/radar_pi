...
void GarminxHDReceive::EmulateFakeBuffer(void) {
  time_t now = time(0);
  UINT8 data[GARMIN_XHD_MAX_SPOKE_LEN];
  ...
  m_next_rotation = (m_next_rotation + 1) % GARMIN_XHD_SPOKES;

  int scanlines_in_packet = GARMIN_XHD_SPOKES * 24 / 60 * MILLIS_PER_SELECT / MILLISECONDS_PER_SECOND;
  ...
  for (int scanline = 0; scanline < scanlines_in_packet; scanline++) {
    ...
    for (size_t range = 0; range < sizeof(data); range++) {
      ...
      if (range > sizeof(data) - 10) {
        colour = ((angle + m_next_rotation) % GARMIN_XHD_SPOKES) <= 8 ? 255 : 0;
      }
      data[range] = colour;
      if (colour > 0) {
        spots++;
...
wxString GarminxHDReceive::GetStatus() { return _("GarminxHD Status OK"); }
